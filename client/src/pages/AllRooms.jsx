...
const AllRooms = () => {
  ...
  const matchesDestination = (room) =>
    !destination || room.hotel?.city?.toLowerCase().includes(destination.toLowerCase());
  const matchesType = (room) =>
    !typeFilter || room.roomType?.toLowerCase() === typeFilter.toLowerCase();
      ...
      {isFilterOpen && (
        <div className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-6 mb-8 flex flex-col md:flex-row gap-10 items-start transition-colors duration-300">
          ...
          <div className="flex flex-col gap-3 justify-end">
            {isFiltered && (
              <button onClick={resetFilters} className="text-sm text-red-500 hover:underline">Reset Filters</button>
            )}
      ...
      <div className="flex-1">
        {filteredRooms.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <button onClick={resetFilters} className="text-sm underline text-black dark:text-white">
              Clear filters
            </button>
          </div>
        )}
        ...
        {filteredRooms.map((room) => (
          <div key={room._id}
            className="flex flex-col md:flex-row gap-6 py-8 border-b border-gray-200 dark:border-gray-700">
